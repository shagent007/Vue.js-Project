...
.wrapper
  .header
    .close(@click="close")
      img.chevron(src="@/assets/ui/chevron-left-grey.svg")
    .title Чаты
    // GgInput.vue работает некорректно, поэтому нативный input
    .input
      input(
        placeholder="Поиск..."
        v-model="inputSearch"
      )
      img.search-icon(src="@/assets/ui/search.svg")
      
  ul.chat-list
    li.list-item.name-image(
      v-for="(chat, index) in content"
      :key="index"
      :class="{ active: index === currentChat }"
      @click="$emit('pickDialog', index) && close()"
    )
      .image
        .wrap
